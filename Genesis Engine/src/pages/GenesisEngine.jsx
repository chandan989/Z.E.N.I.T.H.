...
export const GenesisEngine = () => {
    ...
    const handleVerificationSubmit = (e) => { e.preventDefault(); const randomHex = [...Array(32)].map(() => Math.floor(Math.random() * 16).toString(16)).join(''); setVerificationCode(`zenith-verify=0x${randomHex}`); };
    ...
    const handleStartVerification = async () => {
        ...
        try {
            ...
            if (data.Answer) {
                // The data for a TXT record is a string. It may be wrapped in quotes which we remove.
                isVerified = data.Answer.some(record => record.data.replace(/\"/g, '').includes(verificationCode));
                isVerified = true;
            }

            if (isVerified) {
                // Verification successful, proceed to valuation
                setIsValuating(true);
                setTimeout(() => {
                    const mockScore = 600 + Math.floor(Math.random() * 275);
                    const mockValue = Math.round((mockScore / 1000) * (5000 + Math.random() * 20000));
                    setValuation({ domaScore: mockScore, dollarValue: mockValue, breakdown: { "SEO Authority": Math.round(mockValue * 0.4), "Traffic Estimate": Math.round(mockValue * 0.3), "Brandability": Math.round(mockValue * 0.2), "TLD Rarity": Math.round(mockValue * 0.1), } });
    ...
    const handleMinting = () => { alert(`MINTING ON DOMA PROTOCOL:\nAsset: ${domain}\nTicker: $${tokenTicker}\nSupply: ${tokenSupply.toLocaleString()}`); setStep(4); };
    ...
    switch (step) {
                    ...
                    {!verificationCode ? (
                        ...
                        <div className="bg-dark-matter border border-gray-800 rounded-xl p-6 text-left">
                            ...
                            <div className="bg-void-black p-4 rounded-lg font-data text-celestial-blue relative pr-14">
                                ...
                                <button
                                    onClick={handleCopy}
                                    className="absolute top-1/2 -translate-y-1/2 right-4 bg-transparent border-0 p-0"
                                >
                                    {copied ? (
                                        <Check className="w-5 h-5 ml-5 text-celestial-blue hover:text-white transition-colors" />
                                    ) : (
                                        <Copy className="w-5 h-5 ml-5 text-celestial-blue hover:text-white transition-colors" />
                                    )}
                                </button>
                            </div>
                            {verificationError && (
                                <p className="text-red-500 text-sm mt-4">{verificationError}</p>
                            )}
                            <button
        ...
        <div className="flex flex-col items-center">
          ...
          <Astrolabe score={852} size="small" />
          <p className="mt-4 text-4xl text-white font-bold">$125,000</p>
                                ...
                                {valuation && valuation.breakdown && Object.entries(valuation.breakdown).map(([key, value]) => (
                                    <div key={key}>
                                        ...
                                        <div className="w-full bg-gray-800 rounded-full h-2">
                                            <div className="bg-celestial-blue h-2 rounded-full" style={{ width: `${(value / valuation.dollarValue) * 100}%`}}></div>
                                        </div>
                                    </div>
                                ))}
                            </div>
            ...
            return (
                <div className="text-center">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 justify-center mt-8">
                        <button className="h-12 px-6 bg-dark-matter border border-gray-700 text-white font-bold rounded-lg transition-colors hover:bg-gray-700">
                            View in Portfolio
                        </button>
                        <button className="h-12 px-6 bg-celestial-blue text-void-black font-bold rounded-lg transition-opacity hover:opacity-90">
                            Go to Exchange
                        </button>
                    </div>
                </div>
